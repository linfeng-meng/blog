      ...
      <div class="left">
        <mu-card class="card">
          <ul class="timeline timeline-centered">
            <li v-for="(item, i) in list" :key="i" :class="[item.years ? 'period' : '', 'timeline-item']" @click="goDetail(item._id)">
          ...
          <div v-if="totalCount > pageSize" class="pagination">
            <mu-pagination raised circle :total="totalCount" :current.sync="page" :pageSize.sync="pageSize"
              :pageCount="5" @change="pageChange"></mu-pagination>
          </div>
        </mu-card>
      </div>

        <Right showPosition="归档"></Right>
  </div>
</template>
<script>
