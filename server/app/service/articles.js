...
class ArticlesService extends Service {
  ...
  async getDetails(params) {
    ...
    const data = await ctx.model.Articles.findById({_id:id})
    const prev = await ctx.model.Articles.find({_id:{'$lt':id}}).sort({_id:-1}).limit(1)
    const next = await ctx.model.Articles.find({_id:{'$gt':id}}).sort({_id:1}).limit(1)
    return {
      data: {
        current: data,
        next:next[0],
